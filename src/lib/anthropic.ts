...
`
  ...
  const prompt = `أنت متخصص في قراءة فواتير المشتريات من الصور. قرائتك يجب أن تكون دقيقة وشاملة لجميع العناصر بدون استثناء.

هذه صورة${images.length > 1 ? "صور" : ""} فاتورة مشتريات (سوبر ماركت أو بقالة). اقرأ **كل سطر مشتريات** فيها بعناية عالية جداً واستخرجه. عدم استخراج أي سطر = خسارة مالية مباشرة.
...
تعليمات حرجة جداً:
١. اقرأ **كل** سطر في الفاتورة بدون استثناء، بما فيها:
  - الأسطر بنصوص صغيرة جداً أو قد تبدو مشوهة قليلاً
  - الأسطر ذات الأسعار الكسرية (مثل 0.50 أو 1.25 أو 1.11)
  - الأسطر في حواف الصور أو الزوايا العلوية والسفلية
  - العناصر المكررة (إذا ظهرت نفس السلعة مرتين، استخرجها مرتين - ما تحذفش التكرار هنا)
  - سطور قد تبدو مختصرة أو بدون معلومة كاملة

٢. احرص على دقة الأسعار:
  - استخرج الأرقام بعناية تامة وبدون تقريب
  - إذا ظهر السعر بصيغة عالية الغموض، خذ أفضل تخمين لك

٣. استخراج الكمية:
  - احفظ الكمية كما تظهر (كيلو، قطع، عدد، صندوق، إلخ)
  - إذا كانت الكمية "1.0" فقط بدون وحدة، احفظها كرقم

٤. لا تدمج أو تحذف:
  - لا تدمج سطرين مختلفين في سطر واحد حتى لو كانا متشابهين الاسم
  - لا تحذف أي سطر تعتقد أنه "تكرار" - النظام سيتعامل مع ذلك لاحقاً
  - لا تتجاهل أي سطر مهما بدا صغيراً أو غير مهم

٥. التحقق قبل الإرسال:
  - عد السطور المستخرجة وقارنها بعدد الأسطر التي تراها في الصور
  - إذا بدا أنك فاتتك سطور (خاصة في البداية أو النهاية)، أعد القراءة بعناية أكثر
  - تأكد أن كل سطر له قيمة line_total معقولة
