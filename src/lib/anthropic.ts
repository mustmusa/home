...
`
  ...
  const prompt = `أنت متخصص في قراءة فواتير المشتريات من الصور بدقة عالية جداً. هذه مهمة حرجة - كل سطر مفقود = خسارة مالية مباشرة.

هذه صورة${images.length > 1 ? "صور" : ""} فاتورة مشتريات من سوبر ماركت/بقالة.
...
**صيغة السطر الواحد:**
{
  ...
  "quantity": الكمية (رقم فقط),
  "unit_price": السعر الأصلي للوحدة (الرقم بدون ضريبة),
  "line_total": المجموع = quantity × unit_price (احسبه بدقة),
...
**تعليمات استخراج حرجة:**

١. **قراءة شاملة:**
   - اقرأ كل سطر مرتين إن لزم الأمر
   - لا تترك أي سطر، مهما كان صغيراً أو غامضاً
   - الأسطر في الحواف والزوايا والنصوص الصغيرة = أولوية

٢. **دقة الأسعار:**
   - استخرج الأسعار بالضبط كما تظهر (بدون تقريب)
   - إذا كان السعر غير واضح، اجتهد بأفضل قراءة
   - الأسعار الكسرية (0.50، 1.25، إلخ) = استخرجها بدقة

٣. **معالجة التطابقات:**
   - إذا رأيت نفس السطر مرتين (overlap بين صور) = استخرجه مرتين
   - لا تحذف أي شيء تعتقده "تكرار" - النظام سيتعامل معه
   - كل سطر في الفاتورة = سطر مستقل في الاستخراج

٤. **التحقق النهائي:**
   - عد السطور المستخرجة - يجب تكون قرب 54
   - تأكد أن المجموع الكلي = مجموع جميع line_totals
   - إذا نقصت سطور، أعد القراءة بعناية

٥. **الصيغة المطلوبة:**
   - أعد مصفوفة JSON فقط
   - بدون نصوص أو تعليقات إضافية
   - كل سطر كائن منفصل
