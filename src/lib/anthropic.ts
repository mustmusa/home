...
`
  ...
  // إذا فشلنا في استخراج أي عناصر من Haiku، لا نتابع - هذا يشير لمشكلة في الصورة
  if (itemsList.length === 0) {
    console.warn("⚠️ تحذير: لم يتمكن Haiku من استخراج أي عناصر من الصور - قد تكون الصور غير واضحة");
  }
...
كل سطر في الفاتورة يحتوي على أعمدة (من اليمين لليسار):
...
**مثالان من الفاتورة:**
- 139.99 | 121.73 | 1.0 | دجاج سيراليممجمد → unit_price: 121.73, qty: 1.0, line_total: 121.73
- 11.50 | 10.00 | 1.0 | أرز | → unit_price: 10.00, qty: 1.0, line_total: 10.00

**كيفية استخراج السعر الأساسي:**
- ابحث عن عمودين من الأرقام: واحد أكبر (مع ضريبة) وواحد أصغر (بدون ضريبة)
- **استخدم الرقم الأصغر دائماً** - هو السعر الأساسي (unit_price)
- احسب line_total = quantity × unit_price

⚠️ **تحذيرات حرجة جداً:**
- الفاتورة تحتوي على **بالضبط 54 سطر مشتريات**
- **كل سطر يجب أن يكون له unit_price وquantity - لا تترك أي واحد فارغ**
- إذا لم تجد السعر بوضوح، استخرج أقرب قيمة رقمية تبدو معقولة
- بعض الأسطر قد تظهر مرتين (overlap من الصور)
- استخرج **كل سطر تراه** بدون حذف - النظام سيعالج التطابقات

**صيغة JSON لكل سطر - مهم جداً:**
{
  "item_name": "اسم العنصر بالضبط كما في الفاتورة",
  "quantity": الكمية (رقم موجب من العمود الثالث - إذا غير واضح استخدم 1.0),
  "unit_price": السعر الأساسي بدون ضريبة (رقم موجب من العمود الثاني - **يجب ألا يكون 0 أو null**),
  "line_total": quantity × unit_price (احسبه بدقة = كمية × سعر الوحدة),
  "category": أقرب فئة من: ${JSON.stringify(CATEGORIES)},
  "suggested_request_id": معرّف من القائمة أدناه أو null
}

⚠️ **قاعدة ذهبية: لا تترك unit_price أو quantity فارغة أو تساوي null**
- إذا لم تتمكن من قراءة الرقم بوضوح، استخدم افتراضًا معقولًا:
  - quantity الافتراضية: 1.0
  - unit_price الافتراضي: استخرج من أي عمود يحتوي على أرقام (البحث عن الرقم الأصغر في الصف)
...
١. **تحديد الأعمدة بدقة:**
   - اليمين تماماً (رقم كبير) = المجموع مع الضريبة - **لا تستخدمه أبداً**
   - ثاني عمود من اليمين = السعر الأساسي - **استخرج هذا دائماً**
   - العمود الثالث = الكمية
   - اليسار = الاسم
   - **نصيحة:** السعر الأساسي عادة يكون ~87% من السعر المع الضريبة (بقسمة المع ضريبة ÷ 1.15)

٢. **استخراج شامل 100% من جميع الأسطر:**
   - 🔴 **حرج جداً:** يجب استخراج **بالضبط 54 عنصر**
   - كل سطر يجب أن يحتوي على unit_price و quantity و line_total - **لا مسافات فارغة**
   - اقرأ الصورة من أعلى إلى أسفل، ثم من أسفل إلى أعلى (مرتين)
   - الأسطر في الحواف والزوايا = **أولوية قصوى**
   - الأرقام الصغيرة والكسرية = ركز عليها (0.566، 0.578، 1.25، إلخ)
   - الأسطر المتقطعة أو المرتبطة بصورة أخرى = استخرجها

٣. **دقة الأسعار - حرج:**
   - **كل سطر يجب أن يحتوي على سعر موجب > 0**
   - الأسعار الكسرية بدقة (13.90، 5.21، 8.69، 12.17، إلخ)
   - إذا كان السعر غير واضح، اختر أقرب رقم بدلاً من 0
...
٥. **التحقق النهائي الإجباري:**
   ...
   - **كل سطر يجب أن يحتوي على line_total > 0 (ليس 0)**
   - إذا أقل من 54: ابحث عن الأسطر المفقودة وأضفها
   - المجموع الكلي = جمع جميع line_totals

٦. **الصيغة المطلوبة:**
   - مصفوفة JSON فقط
   - بدون نصوص إضافية
   - 54 كائن منفصل مع unit_price و line_total موجبة دائماً
...
أعد المصفوفة الآن:`;
  ...
  // معالجة ما بعد الاستخراج: تحقق من الأسعار وأصلح المشاكل
  const processedLines = lines
    .filter((l) => l && typeof l.item_name === "string" && l.item_name.trim())
    .map((l) => {
      let quantity = typeof l.quantity === "number" && l.quantity > 0 ? l.quantity : 1;
      let unitPrice = typeof l.unit_price === "number" && l.unit_price > 0 ? l.unit_price : null;
      let lineTotal = typeof l.line_total === "number" ? l.line_total : null;

      // إذا لم يكن هناك line_total ولكن لدينا quantity و unit_price، احسبه
      if (lineTotal === null || lineTotal === 0) {
        if (unitPrice !== null && quantity > 0) {
          lineTotal = unitPrice * quantity;
        } else if (lineTotal !== null && lineTotal > 0) {
          // إذا كان line_total موجود ولكن unit_price غير واضح، استنتج الثمن
          unitPrice = lineTotal / quantity;
        }
      }

      // إذا لم نتمكن من الحصول على unit_price أو line_total، ضع قيمة افتراضية معقولة
      // (هذا سيسمح للمسؤول بمراجعة البيانات وتصحيحها يدويًا في خطوة المراجعة)
      if (unitPrice === null || unitPrice === 0) {
        console.warn(
          `⚠️ تحذير: السطر "${l.item_name}" لم نتمكن من استخراج السعر - سيتم عرضه كـ 0 للمراجعة`,
        );
        unitPrice = 0;
      }
  ...
  // تحقق من نتيجة الاستخراج
  const withMissingPrices = processedLines.filter((l) => l.line_total === 0 || l.unit_price === 0);
  if (withMissingPrices.length > 0) {
    console.warn(`⚠️ ${withMissingPrices.length} عنصر لم نتمكن من استخراج سعره - سيحتاج المسؤول لمراجعتها وتصحيحها`);
  }
...
/**
 * The shop is printed at the head of the receipt but line extraction never
 * asked for it, so every scanned invoice fell back to the word "متجر".
 * Kept as its own call: a failure here must not cost the caller its lines.
 */
export async function readStoreName(
          ...
          content: [
            {
              ...
              source: {
                ...
                media_type: images[0].mediaType as "image/jpeg" | "image/png" | "image/webp",
                data: images[0].base64,
              },
            },
            {
              type: "text" as const,
              text: `ما اسم المتجر أو السوق المطبوع في رأس هذه الفاتورة؟

أعد JSON فقط: {"store":"الاسم"}
- اكتب الاسم التجاري المعروف بالعربية إن أمكن (مثال: أسواق التميمي، بنده، الدانوب، نستو)
- إن لم يظهر اسم واضح، أعد {"store":null}
- لا تخمّن اسماً غير مكتوب في الصورة`,
            },
          ],
        },
      ],
    ...
    const parsed = JSON.parse(match[0]) as { store?: string | null };
    const store = parsed.store?.trim();
    return store || null;
  } catch {
    return null;
  }
