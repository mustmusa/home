...
`
  ...
  const prompt = `هذه صورة${images.length > 1 ? "صور" : ""} فاتورة مشتريات (سوبر ماركت أو بقالة). اقرأ كل سطر مشتريات فيها واستخرجه بدقة.

${multiImageNote}⚠️ مهم جدًا للدقة:
- استخرج الأسعار والمجاميع كما تظهر في الفاتورة مباشرة، بدون تقريب
- في كل سطر قد تظهر 3 أرقام: السعر الأصلي، السعر بعد الخصم، والكمية
- استخدم السعر الأخير (بعد الخصم) وليس الأول
- line_total يجب أن يكون الرقم النهائي للسطر كما هو مكتوب في الفاتورة (لا تحسبه)
- تأكد أن مجموع جميع line_total يطابق المجموع الكلي في الفاتورة

لكل سطر أعد كائن JSON بالشكل:
{
  "item_name": "اسم العنصر كما في الفاتورة (مختصر ومفهوم)",
  "quantity": رقم الكمية أو null إن لم تُقرأ,
  "unit_price": سعر الوحدة (بعد الخصم إن وجد) أو null,
  "line_total": الرقم النهائي للسطر من الفاتورة تماماً (رقم فقط بدون رمز عملة)،
...
قواعد استخراج دقيق:
- اقرأ كل سطر مشتريات بعناية وصرامة (الأرقام بالذات)
- استخرج الأسعار الفعلية النهائية المدفوعة (بعد الخصومات)
- تأكد أن عدد السطور المستخرجة يطابق عدد السطور في الفاتورة
- إذا ظهر مجموع في نهاية الفاتورة (مثل BAL أو TOTAL)، استخدمه للتحقق من صحة استخراجك
- لا تخترع أسطر، ولا تتجاهل أي سطر، ولا تكرر نفس السطر مرتين
